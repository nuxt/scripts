...
// A workspace package does not always declare Nuxt itself. pnpm installs the
// shared Nuxt version at the workspace root, so fall back to that.
function nuxtProvider(pkg, root) {
  const candidates = [
    resolve(pkg.directory, 'node_modules/nuxt/package.json'),
    resolve(root, 'node_modules/nuxt/package.json'),
  ]
...
function collectDependencies(pkg, metrics, catalog, root) {
  const provider = nuxtProvider(pkg, root)
  ...
  for (const [name, declaredRange] of Object.entries(pkg.packageJson.dependencies || {}).sort()) {
    ...
    total.gzipSize += measured.gzipSize
    total.size += measured.size
  }

  // Install weight the package adds on top of what Nuxt already ships.
  if (total.size) {
    metrics.set(`${pkg.relativeDirectory}:dependencies`, {
      ...total,
      id: `${pkg.relativeDirectory}:dependencies`,
      kind: 'output',
      label: `${pkg.name} · runtime dependencies`,
    })
  }
}
...
function collectPackageMetrics(pkg, metrics) {
  ...
  // Group by the directories the build emits, so a new runtime folder is
  // tracked without editing this script.
  for (const name of subdirectories(pkg.distPath))
    addMetric(metrics, pkg, `dist:${name}`, `dist/${cleanLabel(name)}`, walkFiles(resolve(pkg.distPath, name)))

  for (const name of subdirectories(resolve(pkg.distPath, 'runtime')))
    addMetric(metrics, pkg, `runtime:${name}`, `${cleanLabel(name)} runtime`, walkFiles(resolve(pkg.distPath, 'runtime', name)))
...
export function collectSnapshot(root) {
  ...
  for (const pkg of discoverPackages(absoluteRoot)) {
    ...
    collectDependencies(pkg, metrics, catalog, absoluteRoot)
  }
  return metrics
}
...
function countDependencies(metrics) {
  return [...metrics.values()].filter(metric => metric.kind === 'dependency').length
}
...
export function renderReport(base, head, baseLabel = '') {
  ...
  const baseDependencyCount = countDependencies(base)
  const headDependencyCount = countDependencies(head)
  if (headDependencyCount || baseDependencyCount) {
    const difference = headDependencyCount - baseDependencyCount
    const change = base.size === 0
      ? ''
      : difference === 0
        ? ' (no change)'
        : ` (${difference > 0 ? `🔴 +${difference}` : `🟢 ${difference}`} vs ${baseDependencyCount})`
    output.push('', `📚 **${headDependencyCount} runtime dependenc${headDependencyCount === 1 ? 'y' : 'ies'}**${change}`)
  }
  if (changed.length) {
