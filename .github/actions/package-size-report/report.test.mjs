...
function makeRepository(files) {
  ...
  // Nuxt lives at the workspace root, as pnpm installs it in this repository.
  mkdirSync(resolve(root, 'node_modules/nuxt'), { recursive: true })
  ...
  writeFileSync(resolve(root, 'node_modules/nuxt/package.json'), JSON.stringify({
    dependencies: { ofetch: '^1.5.0' },
    version: '4.5.1',
  }))
  mkdirSync(resolve(root, 'node_modules/ofetch'), { recursive: true })
  writeFileSync(resolve(root, 'node_modules/ofetch/package.json'), JSON.stringify({
    files: ['dist'],
    version: '1.5.1',
  }))
...
it('collects exports, runtime groups, and published payload', () => {
  ...
  // Only the dependency Nuxt does not already ship counts as install weight.
  assert.equal(
    snapshot.get('packages/module:dependencies').size,
    snapshot.get('packages/module:dependency:paid-dep').size,
  )
})
...
it('reports growth and removed output against the base build', () => {
  ...
  assert.match(report, /2 runtime dependencies.+no change/)
})
