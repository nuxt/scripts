...
// Privacy presets
export const PRIVACY_NONE: ProxyPrivacyInput = {
    ip: false,
    userAgent: false,
    language: false,
    screen: false,
...
export const PRIVACY_FULL: ProxyPrivacyInput = {
    ip: true,
    userAgent: true,
    language: true,
    screen: true,
...
export const PRIVACY_HEATMAP: ProxyPrivacyInput = {
    ip: true,
    userAgent: false,
    language: true,
    screen: false,
...
export const PRIVACY_IP_ONLY: ProxyPrivacyInput = {
    ip: true,
    userAgent: false,
    language: false,
    screen: false,
...
export const PRIVACY_DESCRIPTIONS: Record<string, PrivacyDescription> = {
    ...
    full: {
        label: 'Full',
        description: 'All identifying data is anonymised: IP, user agent, language, screen, timezone, and hardware.',
    },
    heatmap: {
        label: 'Heatmap',
        description: 'IP, language, and hardware fingerprints anonymised; screen data preserved for heatmap accuracy.',
    },
    none: {
        label: 'None',
        description: 'No data is anonymised. Sensitive auth headers (cookies, authorization) are still stripped.',
    },
};
...
export function getPrivacyDescription(privacy: ProxyPrivacyInput | null | undefined): PrivacyDescription | null {
    if (!privacy || privacy === false) return null;
    if (privacy === true) return PRIVACY_DESCRIPTIONS.full!;
...
function m(
    ...
    composableName: string | false,
    capabilities: ScriptCapabilities,
    privacy: ProxyPrivacyInput | null = null,
): RegistryScriptMeta {
...
export const registryMeta: RegistryScriptMeta[] = [
    ...
    m(
        ...
        { bundle: true, proxy: true, partytown: true },
        PRIVACY_HEATMAP,
    ),
    m(
        'plausibleAnalytics',
...
export function getProxyDef(
    ...
    if (!script.proxy) return undefined;
    if (typeof script.proxy === 'string') return scriptByKey?.get(script.proxy)?.proxy as ProxyCapability | undefined;
    return script.proxy;
}
...
export function resolveAutoInject(ai: ProxyAutoInject): ResolvedProxyAutoInject {
    const computeValue = ai.resolve
        ...
        : ai.target
          ...
          : undefined;
    if (!computeValue) throw new Error(`ProxyAutoInject for field "${ai.field}" needs either "target" or "resolve"`);
...
export function getBundleResolve(script: RegistryScript): ((options?: any) => string | false) | undefined {
    if (!script.bundle || script.bundle === true) return undefined;
...
export async function registry(resolve?: (path: string) => Promise<string>): Promise<RegistryScript[]> {
    ...
    return Promise.all([
            ...
            bundle: {
                resolve: (options?: PlausibleAnalyticsInput) => {
                    ...
                    return options?.extension
                        ? `https://plausible.io/js/script.${extensions}.js`
        ...
        def('rybbitAnalytics', {
            ...
            bundle: {
                resolve: (options?: RybbitAnalyticsInput) => {
                    ...
                    if (host && !host.startsWith('/')) return `${host}/script.js`;
        ...
        def('tiktokPixel', {
            ...
            bundle: {
                resolve(options?: TikTokPixelInput) {
                    ...
                    return withQuery('https://analytics.tiktok.com/i18n/pixel/events.js', {
                        sdkid: options.id,
                        lib: 'ttq',
                    });
                },
        ...
        def('intercom', {
            ...
            bundle: {
                resolve(options?: IntercomInput) {
                    if (!options?.app_id) return false;
        ...
        def('npm', {
            ...
            bundle: {
                resolve(options?: NpmInput) {
                    return withBase(
                        options?.file || '',
                        `https://unpkg.com/${options?.packageName || ''}@${options?.version || 'latest'}`,
                    );
                },
            },
        }),
        ...
        def('gravatar', {
            ...
            serverHandlers: [{ route: '/_scripts/proxy/gravatar', handler: './runtime/server/gravatar-proxy' }],
        }),
    ]);
}
 ...
 */
export function resolveCapabilities(script: RegistryScript, scriptOptions?: Record<string, any>): ScriptCapabilities {
    ...
    if (!scriptOptions) return resolved;
    ...
    for (const key of overrideKeys) {
        if (key in scriptOptions) {
            ...
            if (typeof userValue !== 'boolean') continue;
...
export function buildProxyConfigsFromRegistry(
    ...
    if (!scriptByKey) {
        ...
        for (const script of scripts) {
            if (script.registryKey) scriptByKey.set(script.registryKey, script);
        }
    }

    for (const script of scripts) {
        if (!script.registryKey || !script.proxy) continue;

        const proxyDef = getProxyDef(script, scriptByKey);
        if (!proxyDef?.domains?.length) continue;

        configs[script.registryKey] = {
            domains: proxyDef.domains.map((d) => (typeof d === 'string' ? d : d.domain)),
            privacy: proxyDef.privacy || {
                ip: false,
                userAgent: false,
                language: false,
                screen: false,
