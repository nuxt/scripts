...
describe('speedcurve', { timeout: 15000 }, () => {
  // Block the real CDN so window.LUX spy set up in fixtures is never overwritten
  async function scPage(path: string) {
    return createPage(path, {
      setup: (p: any) => p.route('**/cdn.speedcurve.com/**', (r: any) => r.abort()),
    })
  }
  ...
  // Helper: wait for a specific LUX method to appear in _luxCalls
  async function waitForLuxCall(page: any, method: string, timeoutMs = 5000) {
    await page.waitForFunction(
      (m: string) => ((window as any)._luxCalls as any[])?.some((c: any) => c.method === m),
      method,
      { timeout: timeoutMs },
    )
  }
  ...
  it('SPA auto: timing order is startSoftNavigation before markLoadTime', async () => {
    ...
    await page.evaluate(() => { (window as any)._luxCalls = [] })
  ...
  it('SPA auto default label: uses Nuxt-generated route name as label', async () => {
    ...
    // Nuxt generates a route name from the file path — e.g. 'tpc-speedcurve-destination'.
    // String(to.name ?? to.path) returns this name. Path fallback (to.name === undefined)
    // is unreachable via Nuxt file-based routing — it is covered by unit test instead.
  ...
  it('canceled navigation: seals phantom beacon with luxNavFailed tag', async () => {
    ...
    // The afterEach(failure) handler fires synchronously after the guard blocks
    await page.waitForFunction(
      () => ((window as any)._luxCalls as any[])?.some((c: any) => c.method === 'addData'),
      { timeout: 5000 },
    )
    ...
    // We must still be on the index page (navigation was blocked)
  ...
  it('SPA off: no startSoftNavigation call when spaMode is false', async () => {
    ...
    await page.click('#nav-destination')
    await page.waitForSelector('#page')
    await page.waitForTimeout(1000) // give hooks time to fire if they mistakenly exist
  ...
  it('labelFor false: leaves window.LUX.label unchanged after navigation', async () => {
    ...
    await page.waitForTimeout(500)
    const label = await page.evaluate(() => (window as any).LUX?.label)
    // The fixture initialises label to 'original-label'; with labelFor:false the composable
    // must never overwrite it, regardless of which route was navigated to.
