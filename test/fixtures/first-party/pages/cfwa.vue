<script lang="ts" setup>
import { useHead, useScriptCloudflareWebAnalytics } from '#imports'

useHead({ title: 'Cloudflare WA - First Party' })
const { status } = useScriptCloudflareWebAnalytics({ token: 'ade278253a19413c9bd923b079870902', scriptOptions: { trigger: 'client' } })
</script>

<template>
  <div>
    <h1>Cloudflare Web Analytics First-Party Test</h1>
    <ClientOnly>
      <div id="status">
        status: {{ status }}
      </div>
    </ClientOnly>
    <p>CFWA automatically tracks pageviews. Navigate between pages to trigger events.</p>
    <NuxtLink to="/">
      Go Home (triggers SPA pageview)
    </NuxtLink>
  </div>
</template>
