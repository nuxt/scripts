...
The simplest way to load Google Sign-In globally in your Nuxt App is to use Nuxt config. Alternatively you can directly use the [`useScriptGoogleSignIn()`](#usescriptgooglesignin){lang="ts"} composable.
...
## [`useScriptGoogleSignIn()`](/scripts/google-sign-in){lang="ts"}

The [`useScriptGoogleSignIn()`](/scripts/google-sign-in){lang="ts"} composable lets you have fine-grained control over when and how you load Google Sign-In.
