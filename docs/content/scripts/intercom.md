...
The [`<ScriptIntercom>`](/scripts/intercom){lang="html"} component emits a single `ready` event when Intercom is loaded.
...
The [`useScriptIntercom()`](/scripts/intercom){lang="ts"} composable lets you have fine-grain control over when and how Intercom is loaded on your site.
