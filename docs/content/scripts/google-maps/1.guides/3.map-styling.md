...
::callout{color="amber"}
JSON `styles` and `mapId` are mutually exclusive. When you provide both, the component ignores `mapId` and applies `styles`. Cloud-based styling also requires the map to use `AdvancedMarkerElement` (legacy `Marker` is not supported with map IDs).
::
