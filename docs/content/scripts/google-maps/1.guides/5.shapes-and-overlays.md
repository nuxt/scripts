...
description: Draw circles, polygons, polylines, rectangles, and GeoJSON data on a Google map.
---
...
Place each shape component inside `<ScriptGoogleMaps>`{lang="html"} and pass the corresponding Google Maps API options through `options`. Google's [shapes guide](https://developers.google.com/maps/documentation/javascript/shapes) covers the geometry and styling fields shared by these components.
...
Load GeoJSON from a URL or pass inline data. Replacing `src` reloads the layer, while nested changes to an inline object do not. The component watches the `style` prop deeply.
