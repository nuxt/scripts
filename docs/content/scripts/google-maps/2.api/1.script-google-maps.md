...
|----------|------|-------------|
| `googleMaps` | `ShallowRef<google.maps.MapsLibrary>`{lang="html"} | The core Maps API library. |
| `map` | `ShallowRef<google.maps.Map>`{lang="html"} | The map instance. |
| `createAdvancedMapMarker` | `(options) => AdvancedMarkerElement` | Create markers programmatically. |
