...
MapLibre applies the new options in a worker, so the update finishes later. If the update fails, the component emits `error`. The next option change then rebuilds the source. If a second change starts before the first one finishes, the component ignores a failure from the first change.
