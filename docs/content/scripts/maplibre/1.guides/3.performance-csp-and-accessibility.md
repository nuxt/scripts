...
The component's container is the only landmark. MapLibre marks its canvas as a second `region`, so the component removes that role.

MapLibre's keyboard handler pans, zooms and rotates the map while the canvas has focus. The canvas stays in the tab order only while that handler runs. If keyboard control adds nothing for your users, disable it. The canvas then leaves the tab order and the accessibility tree:

```vue
<ScriptMapLibreMap
  aria-label="Depot locations"
...
The component reads the handler state when it creates the map. If you call `map.keyboard.disable()`{lang="ts"} later, update the canvas attributes yourself.
