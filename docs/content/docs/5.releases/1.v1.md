...
Per-script opt-out uses `reverseProxyIntercept: false` in the script's options.
...
**Custom Marker Content** ([#658](https://github.com/nuxt/scripts/pull/658)): The `#content` slot on `ScriptGoogleMapsAdvancedMarkerElement` replaces the default pin with any HTML or Vue template. Build price tags, status badges, or any custom marker visual declaratively.
...
**Deprecation**: `ScriptGoogleMapsMarker` (legacy `google.maps.Marker`) emits a dev warning. Use `ScriptGoogleMapsAdvancedMarkerElement` for new projects.
