...
First-party mode acts as a **reverse proxy**: scripts are bundled at build time and served from your domain, while runtime collection requests are forwarded through Nitro server routes. Before forwarding, the proxy anonymises identifying data (IPs, user agents, hardware fingerprints) based on per-script privacy tiers. This is **auto-enabled** for all scripts that support it.

What this means in practice is data sent to third-party servers gets anonymised: IPs (180.233.124.74 -> 180.233.124.0) and browser versions (Mozilla/5.0 (compatible; Chrome/120.0)).
