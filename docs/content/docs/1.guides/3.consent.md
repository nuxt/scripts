...
Many third-party scripts include tracking cookies that require user consent under privacy laws. Nuxt Scripts simplifies this process with the [`useScriptTriggerConsent()`](/docs/api/use-script-trigger-consent){lang="ts"} composable, allowing scripts to be loaded only after receiving user consent.

## Usage

The [`useScriptTriggerConsent()`](/docs/api/use-script-trigger-consent){lang="ts"} composable offers flexible interaction options suitable for various scenarios.

See the [API](/docs/api/use-script-trigger-consent) docs for full details on the available options.

### Accepting as a Function

The easiest way to make use of [`useScriptTriggerConsent()`](/docs/api/use-script-trigger-consent){lang="ts"} is by invoking the `accept` method when user consent is granted.
...
Alternatively, you can pass a reactive reference to the consent state to the [`useScriptTriggerConsent()`](/docs/api/use-script-trigger-consent){lang="ts"} composable. This will automatically load the script when the consent state is `true`.
...
For this you can use the `postConsentTrigger`, which shares the same API as `trigger` from the [`useScript()`](/docs/api/use-script){lang="ts"} composable.
