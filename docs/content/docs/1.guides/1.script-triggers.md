...
Try the live [Performance Example](https://stackblitz.com/github/nuxt/scripts/tree/main/examples/performance) on StackBlitz to see triggers in action.
::
...
Use [`useScriptTriggerIdleTimeout()`](/docs/api/use-script-trigger-idle-timeout){lang="ts"} to delay script loading for a specified time after Nuxt is ready:
...
Use [`useScriptTriggerInteraction()`](/docs/api/use-script-trigger-interaction){lang="ts"} to load scripts when users interact with your site:
...
Use [`useScriptTriggerElement()`](/docs/api/use-script-trigger-element){lang="ts"} to trigger scripts based on specific element interactions:
