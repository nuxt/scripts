...
The [`useScript()`](/docs/api/use-script){lang="ts"} composable is the core of Nuxt Scripts and is used to load all scripts.

There are additional layers of abstraction built on top of [`useScript()`](/docs/api/use-script){lang="ts"} to make it easier to load scripts in different ways.
...
For this reason, you may consider wrapping your [`useScript()`](/docs/api/use-script){lang="ts"} calls in their own composable to allow for easier instantiation of the script.
...
You may wonder how the [`useScript()`](/docs/api/use-script){lang="ts"} composable can return SSR safe functions that can be called before the script is loaded.
