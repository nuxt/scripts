...
The reverse proxy requires a **server runtime**. For static deployments (`nuxt generate`), Nuxt Scripts automatically disables the proxy. Scripts are still bundled and served from your domain, but runtime collection requests (analytics beacons, pixel fires) go directly to third-party servers.
...
export default defineNuxtConfig({
  scripts: {
    security: {
      ...
      // stable response etag). SSR/prerendered signed URLs keep working,
      // but client-side reactive proxy calls will be rejected (403).
      pageToken: true,
    }
  }
})
...
| Proxy not working on static site | Nuxt Scripts automatically disables the reverse proxy for SSG. Use platform rewrites or switch to server mode. See [Static Hosting](#static-hosting-ssg) |
