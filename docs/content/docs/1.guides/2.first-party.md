...
**Layer 1: Routing.** Scripts are bundled and served from your domain. Collection requests proxy through your server. Users never connect to third-party servers, so no third-party cookies are set and ad blockers don't interfere.
...
These scripts can't be bundled at build time, but their collection requests are still proxied through your server:
