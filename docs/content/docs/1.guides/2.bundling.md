...
// Force download bypassing cache (useful for development)
useScript('https://example.com/script.js', {
  bundle: 'force',
})
```
...
// Force download for development
useScriptGoogleAnalytics({
  id: 'GA_MEASUREMENT_ID',
  scriptOptions: {
    bundle: 'force'
...
**Note**: Using `'force'` will re-download scripts on every build, which may significantly increase build time. This is primarily useful during development when you want to ensure you always have the latest version of a script.
...
export default defineNuxtConfig({
  scripts: {
    defaultScriptOptions: {
      bundle: true, // or 'force' for development
    }
  }
})
```
