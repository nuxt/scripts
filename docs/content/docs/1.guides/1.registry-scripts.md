...
You can configure registry scripts through your `.env` file, allowing you to provide script options without hardcoding them in your codebase.
...
When modifying the key, any environment variables you're using will break.
...
Registry scripts doesn't stop you from making use of the core [`useScript()`{lang="ts"}](/docs/api/use-script){lang="ts"} features, you can provide extra options to opt-in to the advanced features.
