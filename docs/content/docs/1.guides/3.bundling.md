...
During the build process, your code is checked to find any instances of [`useScript()`](/docs/api/use-script){lang="ts"} that need to be bundled.
...
Subresource Integrity (SRI) is a security feature that ensures scripts haven't been tampered with. According to the [W3C SRI specification](https://www.w3.org/TR/SRI/), it allows browsers to verify that resources fetched are delivered without unexpected manipulation. When enabled, a cryptographic hash is calculated for each bundled script and added as an `integrity` attribute.
