...
- What the [`useScriptNpm()`](/scripts/npm){lang="ts"} registry script is.
...
## Background on [`useScriptNpm()`](/scripts/npm){lang="ts"}

To load the script, we'll be using the [`useScriptNpm()`](/scripts/npm){lang="ts"}.

This is a [registry script](/scripts), a supported
third-party integration built on top of the
[`useScript()`](/docs/api/use-script){lang="ts"} composable that allows you to load scripts from [npm](https://npmjs.com).
...
The [`useScriptNpm()`](/scripts/npm){lang="ts"} registry script abstracts this process, allowing you to load scripts that export immediately invokable functions,
...
Within one of your components, you'll want to load the script. You can do this by using the [`useScriptNpm()`](/scripts/npm){lang="ts"} registry script.
...
You can use the generic from the [`useScriptNpm()`](/scripts/npm){lang="ts"} composable to add types to the script and add global types to the window object.
...
You can also use the [`useScriptTriggerElement()`](/docs/api/use-script-trigger-element){lang="ts"} composable to trigger loading based on element interactions.
...
In this tutorial, you learned how to load the `js-confetti` script using the [`useScriptNpm()`](/scripts/npm){lang="ts"} registry script.
