...
leverage the [use](/api/use-script#use) function.
...
See the [Understanding proxied functions](/guides/script-loading#Understanding-proxied-functions) section for more information.
...
In this example we'll combine the [useElementScriptTrigger](/api/use-element-script-trigger) composable with the `useScriptNpm` composable to load the script after a certain element is in view.
