...
By default, proxy is auto-enabled when any configured script has the `reverseProxyIntercept` capability. Set to `false` to globally disable all proxying.
...
Each registry script declares its own capabilities. Users can override per-script:
