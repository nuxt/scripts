...
- `'preload'` inserts a `<link rel="preload">` tag, downloading the script early. Best for scripts that load soon after page load.
- `'preconnect'` establishes an early connection (DNS + TCP + TLS) to the script's origin. Use for scripts loaded later.
- `'dns-prefetch'` resolves just DNS. Lightest option, useful for scripts that may or may not load.
