...
::callout{icon="i-heroicons-exclamation-triangle" color="amber"}
**Google Analytics 4** - GA4 has [known issues](https://github.com/BuilderIO/partytown/issues/583) with Partytown. The `navigator.sendBeacon` and `fetch` APIs used for collect requests don't work reliably from web workers. Consider using simpler analytics like **Plausible**, **Fathom**, or **Umami** which are more Partytown-friendly.
::

::callout{icon="i-heroicons-x-circle" color="red"}
**Google Tag Manager** - GTM is **not compatible** with Partytown. GTM dynamically injects scripts and requires full DOM access to function. If you need GTM, load it on the main thread instead. For simple analytics, use individual scripts (Plausible, Fathom) directly with Partytown.
::
