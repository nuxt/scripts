...
Default options for scripts. See the [`useScript()`](/docs/api/use-script){lang="ts"} documentation for more details.
...
Global scripts that should be loaded on all pages. This is a configuration for the [`useScript()`](/docs/api/use-script){lang="ts"} composable.
