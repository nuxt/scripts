...
The built-in static map placeholder was extracted into a standalone [`<ScriptGoogleMapsStaticMap>`{lang="html"}](/scripts/google-maps/api/static-map) component. The following props were removed from `<ScriptGoogleMaps>`{lang="html"}:
...
The new component can also be used standalone for store locators, contact pages, and directions previews without loading the interactive Maps API.
